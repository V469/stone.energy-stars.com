...
! defined( 'WPCPT_VERSION' ) && define( 'WPCPT_VERSION', '3.0.7' );
			...
			class WPCleverWpcpt {
				...
				function __construct() {
					...
					self::$columns = [
						...
						'category'          => esc_html__( 'Category', 'wpc-product-table' ),
						'tag'               => esc_html__( 'Tag', 'wpc-product-table' ),
				...
				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-product-table', false, basename( WPCPT_DIR ) . '/languages/' );
                    ...
                    <div class="wpclever_settings_page wrap">
                        ...
                        <div class="wpclever_settings_page_desc about-text">
                            <p>
                                ...
                                <br/>
                                <a href="<?php echo esc_url( WPCPT_REVIEWS ); ?>"
                                   target="_blank"><?php esc_html_e( 'Reviews', 'wpc-product-table' ); ?></a> |
                                <a href="<?php echo esc_url( WPCPT_CHANGELOG ); ?>"
                                   target="_blank"><?php esc_html_e( 'Changelog', 'wpc-product-table' ); ?></a> |
                                <a href="<?php echo esc_url( WPCPT_DISCUSSION ); ?>"
                                   target="_blank"><?php esc_html_e( 'Discussion', 'wpc-product-table' ); ?></a>
                            </p>
                        </div>
                        ...
                        <div class="wpclever_settings_page_nav">
                            <h2 class="nav-tab-wrapper">
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcpt&tab=how' ) ); ?>"
                                   class="<?php echo esc_attr( $active_tab === 'how' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'How to use?', 'wpc-product-table' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcpt&tab=settings' ) ); ?>"
                                   class="<?php echo esc_attr( $active_tab === 'settings' ? 'nav-tab nav-tab-active' : 'nav-tab' ); ?>">
									<?php esc_html_e( 'Settings', 'wpc-product-table' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wpc_product_table' ) ); ?>"
                                   class="nav-tab">
									<?php esc_html_e( 'Product Tables', 'wpc-product-table' ); ?>
                                </a>
                                <a href="<?php echo esc_url( admin_url( 'admin.php?page=wpclever-wpcpt&tab=premium' ) ); ?>"
							...
							<?php } elseif ( $active_tab === 'premium' ) { ?>
                                <div class="wpclever_settings_page_content_text">
                                    <p>
                                        ...
                                        <a href="https://wpclever.net/downloads/product-table?utm_source=pro&utm_medium=wpcpt&utm_campaign=wporg"
                                           target="_blank">https://wpclever.net/downloads/product-table</a>
                                    </p>
                                    <p><strong>Extra features for Premium Version:</strong></p>
                                    <ul style="margin-bottom: 0">
                                        <li>- Add columns with custom text/shortcode, custom field, or custom
                                            attribute.
                                        </li>
				...
				function get_table_data( $id ) {
					...
					$data['layout']        = get_post_meta( $id, 'layout', true ) ?: 'default';
				...
				function shortcode( $attrs ) {
					...
					if ( $attrs['id'] ) {
						...
						if ( $_products && ( count( $_products ) > 0 ) && is_array( $data['columns'] ) && ( count( $data['columns'] ) > 0 ) ) {
							...
							$table_classes = 'display wpcpt_table wpc_product_table' . ( ! empty( $data['style'] ) ? ' wpc_product_table_style_' . $data['style'] : ' wpc_product_table_style_default' ) . ( ! empty( $data['layout'] ) ? ' wpc_product_table_layout_' . $data['layout'] : ' wpc_product_table_layout_default' ) . ( ! empty( $data['extra_classes'] ) ? ' ' . $data['extra_classes'] : '' );
                            ...
                            <div class="wpcpt_wrapper wpc_product_table_wrapper wpc_product_table_wrapper_<?php echo esc_attr( $attrs['id'] ); ?> wpc_product_table_wrapper_style_<?php echo esc_attr( ! empty( $data['style'] ) ? $data['style'] : 'default' ); ?>">
                                <table class="<?php echo esc_attr( apply_filters( 'wpcpt_table_class', $table_classes, $data ) ); ?>"
                                       data-language="<?php echo esc_attr( htmlspecialchars( json_encode( $data['localization'] ), ENT_QUOTES, 'UTF-8' ) ); ?>"
                                       data-page-length="<?php echo esc_attr( $data['page_length'] ); ?>">
                                    <thead>
											...
											foreach ( $data['columns'] as $column ) {
												...
												switch ( $column['type'] ) {
													...
													case 'review':
														...
														break;
													case 'category':
														$category_list = wc_get_product_category_list( $_product_id );
														$td_content    = '<td class="' . $td_class . '" data-sort="' . esc_attr( wp_strip_all_tags( $category_list ) ) . '">' . $category_list . '</td>';

														break;
													case 'tag':
														$tag_list   = wc_get_product_tag_list( $_product_id );
														$td_content = '<td class="' . $td_class . '" data-sort="' . esc_attr( wp_strip_all_tags( $tag_list ) ) . '">' . $tag_list . '</td>';
				...
				function shortcode_callback( $post ) {
					echo '<div class="wpcpt_shortcode_txt"><input type="text" class="wpcpt_shortcode_input" data-id="' . $post->ID . '" readonly value="[wpcpt id=&quot;' . $post->ID . '&quot; name=&quot;' . esc_attr( $post->post_title ) . '&quot;]"/></div>';
				...
				function configuration_callback( $post ) {
                    ...
                    <div class="wpcpt_configuration wpcpt_configuration_source">
                        <table class="wpcpt_configuration_table">
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_products">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <div class="wpcpt-product-search-wrapper">
                                        <input id="" class="wpcpt-product-search-input"
                                               name="wpcpt_configuration_include_products" type="hidden"
                                               value="<?php echo esc_attr( $data['include'] ); ?>"/>
                                        <label>
                                            <select class="wc-product-search wpcpt-product-search" data-sortable="true"
                                                    multiple="multiple"
                                                    data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wpc-product-table' ); ?>"
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_categories_tags">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <div class="wpcpt-category-search-wrapper">
                                        <input id="" class="wpcpt-category-search-input"
                                               name="wpcpt_configuration_categories" type="hidden"
                                               value="<?php echo esc_attr( $data['categories'] ); ?>"/>
                                        <label>
                                            <select class="wc-category-search wpcpt-category-search" multiple="multiple"
                                                    data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'wpc-product-table' ); ?>">
												<?php
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_categories_tags">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input name="wpcpt_configuration_tags" type="text" style="width: 100%"
                                               placeholder="<?php esc_attr_e( 'Add some tags, split by a comma...', 'wpc-product-table' ); ?>"
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_terms">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <input name="wpcpt_configuration_terms" class="wpcpt_configuration_terms"
                                           type="hidden" style="width: 100%"
                                           value="<?php echo esc_attr( $data['terms'] ); ?>"/>
									<?php
									$terms = $data['terms'];
									...
									?>
                                    <label>
                                        <select class="wpcpt_configuration_terms_select" multiple="multiple"
                                                data-<?php echo esc_attr( $data['type'] ); ?>="<?php echo esc_attr( implode( ',', $terms ) ); ?>">
											<?php
											if ( ! empty( $terms ) ) {
												foreach ( $terms as $t ) {
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_categories_tags">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <div class="wpcpt-product-search-wrapper">
                                        <input id="" class="wpcpt-product-search-input"
                                               name="wpcpt_configuration_exclude_products" type="hidden"
                                               value="<?php echo esc_attr( $data['exclude'] ); ?>"/>
                                        <label>
                                            <select class="wc-product-search wpcpt-product-search" data-sortable="true"
                                                    multiple="multiple"
                                                    data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'wpc-product-table' ); ?>"
                            ...
                            <tr class="wpcpt_configuration_tr wpcpt_configuration_type_row wpcpt_configuration_type_categories_tags wpcpt_configuration_type_on_sale wpcpt_configuration_type_best_selling wpcpt_configuration_type_terms">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input name="wpcpt_configuration_limit" type="number" class="text small-text"
                                               placeholder="50" value="<?php echo esc_attr( $data['limit'] ); ?>"/>
                                    </label>
                                </td>
                            </tr>
                    ...
                    <div class="wpcpt_configuration wpcpt_configuration_design" style="display: none">
                        <table class="wpcpt_configuration_table">
                            ...
                            <tr class="wpcpt_configuration_tr">
                                <td class="wpcpt_configuration_th">
									<?php esc_html_e( 'Layout', 'wpc-product-table' ); ?>
                                </td>
                                <td class="wpcpt_configuration_td">
                                    <label> <select name="wpcpt_configuration_layout">
                                            <option value="default" <?php selected( $data['layout'], 'default' ); ?>><?php esc_html_e( 'DataTables (default)', 'wpc-product-table' ); ?></option>
                                            <option value="simple" <?php selected( $data['layout'], 'simple' ); ?>><?php esc_html_e( 'Simple', 'wpc-product-table' ); ?></option>
                                        </select> </label>
									<?php esc_html_e( 'Read more about DataTables here: https://datatables.net', 'wpc-product-table' ); ?>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                <td class="wpcpt_configuration_th">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="number" class="text small-text" min="1" step="1"
                                               name="wpcpt_configuration_page_length"
                                               value="<?php echo esc_attr( $data['page_length'] ); ?>"
                                               placeholder="10"/>
                                    </label>
									<?php esc_html_e( 'For DataTables only.', 'wpc-product-table' ); ?>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                <td class="wpcpt_configuration_th">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_configuration_extra_classes"
                                               value="<?php echo esc_attr( $data['extra_classes'] ); ?>"/>
                                    </label>
                    ...
                    <div class="wpcpt_configuration wpcpt_configuration_localization" style="display: none">
                        ...
                        <table class="wpcpt_configuration_table">
                            ...
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[decimal]"
                                               value="<?php echo esc_attr( $localization['decimal'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['decimal']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[emptyTable]"
                                               value="<?php echo esc_attr( $localization['emptyTable'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['emptyTable']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[info]"
                                               value="<?php echo esc_attr( $localization['info'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['info']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[infoEmpty]"
                                               value="<?php echo esc_attr( $localization['infoEmpty'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['infoEmpty']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[infoFiltered]"
                                               value="<?php echo esc_attr( $localization['infoFiltered'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['infoFiltered']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[infoPostFix]"
                                               value="<?php echo esc_attr( $localization['infoPostFix'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['infoPostFix']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[thousands]"
                                               value="<?php echo esc_attr( $localization['thousands'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['thousands']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[lengthMenu]"
                                               value="<?php echo esc_attr( $localization['lengthMenu'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['lengthMenu']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[loadingRecords]"
                                               value="<?php echo esc_attr( $localization['loadingRecords'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['loadingRecords']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[processing]"
                                               value="<?php echo esc_attr( $localization['processing'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['processing']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[search]"
                                               value="<?php echo esc_attr( $localization['search'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['search']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[zeroRecords]"
                                               value="<?php echo esc_attr( $localization['zeroRecords'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['zeroRecords']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[paginate][first]"
                                               value="<?php echo esc_attr( $localization['paginate']['first'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['paginate']['first']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[paginate][last]"
                                               value="<?php echo esc_attr( $localization['paginate']['last'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['paginate']['last']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[paginate][next]"
                                               value="<?php echo esc_attr( $localization['paginate']['next'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['paginate']['next']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[paginate][previous]"
                                               value="<?php echo esc_attr( $localization['paginate']['previous'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['paginate']['previous']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[aria][sortAscending]"
                                               value="<?php echo esc_attr( $localization['aria']['sortAscending'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['aria']['sortAscending']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                            <tr class="wpcpt_configuration_tr">
                                ...
                                <td class="wpcpt_configuration_td">
                                    <label>
                                        <input type="text" name="wpcpt_localization[aria][sortDescending]"
                                               value="<?php echo esc_attr( $localization['aria']['sortDescending'] ?? '' ); ?>"
                                               placeholder="<?php echo self::$localization['aria']['sortDescending']; ?>"/>
                                    </label>
                                </td>
                            </tr>
                        </table>
                        ...
                        <div class="wpcpt-column-content">
                            <div class="wpcpt-column-line">
                                ...
                                <div class="wpcpt-column-line-content">
                                    <input type="hidden" name="wpcpt_columns[<?php echo esc_attr( $key ); ?>][type]"
                                           value="<?php echo esc_attr( $col['type'] ); ?>"/>
                                    <label>
                                        <input type="text" name="wpcpt_columns[<?php echo esc_attr( $key ); ?>][title]"
                                               style="width: 100%" placeholder="<?php echo esc_attr( $col['type'] ); ?>"
                                               value="<?php echo esc_attr( $col['title'] ); ?>" required/>
                                    </label>
                                </div>
                            </div>
							<?php
                        ...
                        <label> <select class="wpcpt-column-type">
                                ...
                                <optgroup label="<?php esc_attr_e( 'Custom (Premium)', 'wpc-product-table' ); ?>">
                                    <option value="custom_field"
                                            disabled><?php esc_html_e( 'Custom field', 'wpc-product-table' ); ?></option>
                                    <option value="custom_attribute"
                                            disabled><?php esc_html_e( 'Custom attribute', 'wpc-product-table' ); ?></option>
                                    <option value="custom"
                                            disabled><?php esc_html_e( 'Custom text/shortcode', 'wpc-product-table' ); ?></option>
                                </optgroup>
                            </select> </label>
                        <input type="button" class="button wpcpt-column-new"
                               value="<?php esc_attr_e( '+ Add new column', 'wpc-product-table' ); ?>"/>
                    </div>
					<?php
				}
				...
				function table_custom_column( $column, $postid ) {
					if ( $column === 'wpcpt_shortcode' ) {
						echo '<input type="text" class="wpcpt_shortcode_input" readonly value="[wpcpt id=&quot;' . $postid . '&quot; name=&quot;' . esc_attr( get_the_title( $postid ) ) . '&quot;]"/>';
					}
				...
				function save_product_table( $post_id ) {
					...
					if ( isset( $_POST['wpcpt_configuration_layout'] ) ) {
						update_post_meta( $post_id, 'layout', sanitize_text_field( $_POST['wpcpt_configuration_layout'] ) );
					}
