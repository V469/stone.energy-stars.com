...
! defined( 'WPCUV_VERSION' ) && define( 'WPCUV_VERSION', '1.2.0' );
...
! defined( 'WPCUV_DIR' ) && define( 'WPCUV_DIR', plugin_dir_path( __FILE__ ) );
		...
		if ( ! class_exists( 'WPCleverWpcuv' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcuv {
				...
				function __construct() {
					...
					// Load variation
					add_action( 'wc_ajax_wpcuv_load_variation', [ $this, 'ajax_load_variation' ] );

					// Update variation
					add_action( 'wc_ajax_wpcuv_update_variation', [ $this, 'ajax_update_variation' ] );
				}

				function init() {
					// load text-domain
					load_plugin_textdomain( 'wpc-update-variations-in-cart', false, basename( WPCUV_DIR ) . '/languages/' );
				}
                            ...
                            <table class="wpcuv-editor">
                                <tr>
                                    ...
                                    <td class="wpcuv-info">
                                        ...
                                        <form class="variations_form wpcuv_variations_form cart" method="post"
                                              enctype='multipart/form-data'
                                              data-product_id="<?php echo absint( $variable_product->get_id() ); ?>"
                                            ...
                                            <div class="single_variation_wrap">
                                                ...
                                                <div class="woocommerce-variation-add-to-cart variations_button woocommerce-variation-add-to-cart-enabled">
                                                    <div class="wpcuv-actions">
														...
														?>
                                                        <button type="submit"
                                                                class="single_add_to_cart_button wpcuv-update button">
															<?php echo esc_html( $update_button_text ); ?>
                                                        </button>
                                                        <span class="button button-primary wpcuv-cancel"
                                                              data-key="<?php echo esc_attr( $current_key ); ?>"><?php echo esc_html( $cancel_button_text ); ?></span>
                                                    </div>
                                                    <input type="hidden" class="product_thumbnail"
                                                           value="<?php echo htmlentities( $variable_product->get_image() ); ?>"/>
                                                    <input type="hidden" name="add-to-cart"
                                                           value="<?php echo absint( $variable_product->get_id() ); ?>"/>
                                                    <input type="hidden" name="product_id"
                                                           value="<?php echo absint( $variable_product->get_id() ); ?>"/>
                                                    <input type="hidden" name="variation_id" class="variation_id"
                                                           value="0"/>
                                                    <input name="old_key" class="old_key" type="hidden"
